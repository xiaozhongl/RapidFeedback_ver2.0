...
public class LoginServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			if (login_ACK > 0) {
				...
				projectList = dbFunction.getProjectList(login_ACK);
				String projectListString = JSON.toJSONString(projectList);
				int id = login_ACK;
				String token = newToken(request, id);
				servletContext.setAttribute(token, id);
				String firstName = dbFunction.getMarkerName(id);
	...
	private String newToken(HttpServletRequest request, int id ) {
